// ─── API Base URL ───────────────────────────────────────────────
// Read from window.ENV injected by index.html <script> block.
// In production (Vercel) this comes from the BACKEND_URL env var.
// In local development it falls back to localhost:5000.
const API_BASE = (window.ENV && window.ENV.BACKEND_URL)
  ? window.ENV.BACKEND_URL.replace(/\/$/, '')   // strip trailing slash
  : 'http://localhost:5000';
...
async function analyze() {
  ...
  try {
    ...
    showError(`⚠ Cannot reach server (${API_BASE}). Check your backend is running.`);
  }
...
function renderSeverity(severity) {
  ...
  const map = {
    low:      { width: '25%',  bg: '#22c55e' },
    moderate: { width: '60%',  bg: '#f59e0b' },
    high:     { width: '95%',  bg: '#ef4444' },
  };
...
async function downloadPDF() {
  ...
  try {
    ...
    if (recSections.length === 0) {
      const lines = doc.splitTextToSize(clean(recContainer?.innerText || ''), cW - 4);
